...
void from_json(const nlohmann::json &j, Message &message)
{
    ...
    try {
        ...
        converted = false;
        api->Log(ELogLevel_INFO, addon_name, "assuming short_message wasn't converted...");
    ...
    try {
        ...
        converted = false;
        api->Log(ELogLevel_INFO, addon_name, "assuming message wasn't converted...");
...
void to_json(nlohmann::json &j, const Message &message)
{
    ...
    j["short_message"] = converter.from_bytes(message.short_message);
    j["message"] = converter.from_bytes(message.message);
